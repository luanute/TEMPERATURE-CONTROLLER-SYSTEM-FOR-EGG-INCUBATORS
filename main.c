...
void PID(){
		if (Get_Millis() - thoigian >= 1500) {
    timePrev = Time;
    thoigian = Get_Millis();
    PID_error = setpoint - avg_temperature;               // Sai số: mục tiêu – thực tế

    if (PID_error > 4.1) { PID_i = 0; }                     // Chặn I-term với sai số lớn (>3.1°C)
    ...
    if ((PT100_Temperature >= setpoint - 0.5) && fix == 0) {
        GPIO_Write(GPIOA,15, 0);  // Kích mạch SSR 
        delay_ms(3000);                          // Giữ 1 s
    ...
    if (PT100_Temperature >= setpoint && fix_oke == 1) {
        ...
        GPIO_Write(GPIOA, 15, 1);  // Ngắt SSR (off)
    }
