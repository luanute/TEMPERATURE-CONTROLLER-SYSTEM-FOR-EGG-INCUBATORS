 ...
 */
 ...
 //SPI_SendData(MAX31865_SPI, &read_reg_addr, 1);
    //SPI_MasterReceiveData(MAX31865_SPI, &MAX31865_Config_Value, 1); 
...
#define MAX31865_Rref (double)388.0  //Сопротивление референсного резистора, подключенного к MAX31865
/*--------------Характеристики датчика типа PT100 и референсный резистор, подключенный к MAX31865------------*/
...
/*-------------------------------------------Для работы по spi-----------------------------------------------*/
#define cs_set() CS_GPIO_Port-> BSRR = (uint32_t) 1 << (16+4); //CS_притягиваем к земле
#define cs_reset() CS_GPIO_Port ->BSRR = 1<< 4; //CS притягиваем к 3.3v
SPI_TypeDef *MAX31865_SPI = SPI1;
...
void MAX31865_Init(uint8_t num_wires) {
	...
	cs_set();
	SPI_SendData(MAX31865_SPI, MAX31865_Configuration_register_write, 2);
	cs_reset();
...
uint8_t MAX31865_Configuration_info(void) {
	...
	cs_set();
	SPI_SendData(MAX31865_SPI, &read_data, 1);
	SPI_MasterReceiveData(MAX31865_SPI, &MAX31865_Configuration, 1); 
	cs_reset();
...
double MAX31865_Get_Temperature(void) {
	...
	SPI_SendData(MAX31865_SPI, &MAX31865_start_address_of_the_poll, 1);
	SPI_MasterReceiveData(MAX31865_SPI, MAX31865_rx_buffer, 7); 
	cs_reset();
